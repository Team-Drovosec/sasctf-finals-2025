...
__attribute__((noinline)) void unsafe_symbolic_table_bounded_contains_symbolic_ptr()
{
    uint8_t data[16];
    ...
    // Create a symbolic pointer by xoring symbolic data with a constant base pointer
    char* symbolic_ptr = (char*)((uint64_t)(*(volatile char**)&data[0]) ^ (uint64_t)g_Data);
    uint32_t symbolic_var = *(volatile uint32_t*)&data[4];
...
{
    unsafe_symbolic_table_bounded_contains_symbolic_ptr();
    ENTRY_POINT_END();
}
