...
__attribute__((noinline)) void safe_symbolic_table_multiple_levels2()
{
    uint8_t data[32];
    ...
    uint32_t symbolic_index_outer0 = verifier_ranged_u32(*(volatile uint32_t*)&data[0], 0, 2);
    uint32_t symbolic_index_outer1 = verifier_ranged_u32(*(volatile uint32_t*)&data[4], 1, 3);
...
{
    safe_symbolic_table_multiple_levels2();
    ENTRY_POINT_END();
}
